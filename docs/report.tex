...
\begin{enumerate}[noitemsep]
    ...
    \item Otherwise, the protected application is intercepted. The application's windows are hidden using \texttt{NSRunningApplication.hide()}, and a \texttt{SIGSTOP} UNIX signal is immediately sent to its Process ID (PID) via \texttt{kill(pid, SIGSTOP)}. This completely freezes the application in memory, preventing any background processing, network requests, or interactions.
    \item A floating authentication panel (\texttt{NSPanel} with \texttt{.floating} window level) is presented.
    \item Upon successful authentication, a \texttt{SIGCONT} signal is sent to the PID to unfreeze the process, and the app is unhidden via \texttt{NSRunningApplication.unhide()}.
    \item If authentication fails, the application remains safely frozen and hidden. No unsaved user data is lost.
\end{enumerate}

\subsection{macOS API Constraints}
\begin{itemize}[noitemsep]
    \item The application utilizes POSIX signals to suspend and resume external processes, which provides true OS-level locking.
    \item The application must be \textbf{non-sandboxed} to control other processes. Sandboxed apps are prohibited from modifying external process states.
...
To avoid requiring authentication on every interaction, FaceLock Pro implements robust session management.

\subsection{Per-App Session Tracking}
Sessions are tracked individually for each protected application bundle identifier using a timestamp dictionary (\texttt{[String: Date]}). This ensures that unlocking one application (e.g., Safari) does not inadvertently unlock another (e.g., Mail). 

Additionally, the system implements a \textbf{3-second grace period} following a successful authentication. When an app is unlocked and programmatically brought to the foreground via \texttt{app.activate()}, macOS fires a subsequent \texttt{didActivate} notification. The grace period instructs the monitor to ignore activation events within 3 seconds of a successful authentication, completely eliminating infinite authentication loops without compromising security.
...
\begin{enumerate}
    ...
    \item \textbf{Camera Warm-up Delay} — The pipeline explicitly drops the first 10 frames to allow the hardware sensor to auto-adjust exposure and white balance, preventing corrupted embeddings from underexposed frames.
    ...
    \item \textbf{Quality Assessment} — \texttt{VNDetectFaceCaptureQualityRequest} scores the face quality. If the score is below 0.5, the user is prompted to improve lighting.
